use utf8;
package Vicidial::Schema::Result::CallcardLog;

# Created by DBIx::Class::Schema::Loader
# DO NOT MODIFY THE FIRST PART OF THIS FILE

=head1 NAME

Vicidial::Schema::Result::CallcardLog

=cut

use strict;
use warnings;

use Moose;
use MooseX::NonMoose;
use MooseX::MarkAsMethods autoclean => 1;
extends 'DBIx::Class::Core';

=head1 COMPONENTS LOADED

=over 4

=item * L<DBIx::Class::InflateColumn::DateTime>

=back

=cut

__PACKAGE__->load_components("InflateColumn::DateTime");

=head1 TABLE: C<callcard_log>

=cut

__PACKAGE__->table("callcard_log");

=head1 ACCESSORS

=head2 uniqueid

  data_type: 'varchar'
  is_nullable: 0
  size: 20

=head2 card_id

  data_type: 'varchar'
  is_nullable: 1
  size: 20

=head2 balance_minutes_start

  data_type: 'smallint'
  default_value: 3
  is_nullable: 1

=head2 call_time

  data_type: 'datetime'
  datetime_undef_if_invalid: 1
  is_nullable: 1

=head2 agent_time

  data_type: 'datetime'
  datetime_undef_if_invalid: 1
  is_nullable: 1

=head2 dispo_time

  data_type: 'datetime'
  datetime_undef_if_invalid: 1
  is_nullable: 1

=head2 agent

  data_type: 'varchar'
  default_value: (empty string)
  is_nullable: 1
  size: 20

=head2 agent_dispo

  data_type: 'varchar'
  default_value: (empty string)
  is_nullable: 1
  size: 6

=head2 agent_talk_sec

  data_type: 'mediumint'
  default_value: 0
  is_nullable: 1

=head2 agent_talk_min

  data_type: 'mediumint'
  default_value: 0
  is_nullable: 1

=head2 phone_number

  data_type: 'varchar'
  is_nullable: 1
  size: 18

=head2 inbound_did

  data_type: 'varchar'
  is_nullable: 1
  size: 18

=cut

__PACKAGE__->add_columns(
  "uniqueid",
  { data_type => "varchar", is_nullable => 0, size => 20 },
  "card_id",
  { data_type => "varchar", is_nullable => 1, size => 20 },
  "balance_minutes_start",
  { data_type => "smallint", default_value => 3, is_nullable => 1 },
  "call_time",
  {
    data_type => "datetime",
    datetime_undef_if_invalid => 1,
    is_nullable => 1,
  },
  "agent_time",
  {
    data_type => "datetime",
    datetime_undef_if_invalid => 1,
    is_nullable => 1,
  },
  "dispo_time",
  {
    data_type => "datetime",
    datetime_undef_if_invalid => 1,
    is_nullable => 1,
  },
  "agent",
  { data_type => "varchar", default_value => "", is_nullable => 1, size => 20 },
  "agent_dispo",
  { data_type => "varchar", default_value => "", is_nullable => 1, size => 6 },
  "agent_talk_sec",
  { data_type => "mediumint", default_value => 0, is_nullable => 1 },
  "agent_talk_min",
  { data_type => "mediumint", default_value => 0, is_nullable => 1 },
  "phone_number",
  { data_type => "varchar", is_nullable => 1, size => 18 },
  "inbound_did",
  { data_type => "varchar", is_nullable => 1, size => 18 },
);

=head1 PRIMARY KEY

=over 4

=item * L</uniqueid>

=back

=cut

__PACKAGE__->set_primary_key("uniqueid");


# Created by DBIx::Class::Schema::Loader v0.07036 @ 2013-10-01 13:24:18
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:KieqyJO0CadxHa/4vCM6Aw


# You can replace this text with custom code or comments, and it will be preserved on regeneration
__PACKAGE__->meta->make_immutable;
1;
